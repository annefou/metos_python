...
What does it really mean when we say that a netCDF file is self-describing? Let's find out with an example "pres.sfc.2016.nc". If you haven't downloaded this file yet:


~~~
$ curl ftp://ftp.cdc.noaa.gov/Datasets/ncep.reanalysis.dailyavgs/surface/pres.sfc.2016.nc -o pres.sfc.2016.nc
~~~
{: .bash}
...
~~~
$ ncdump -h metos_python/datapres.sfc.2016.nc
~~~
{: .bash}


~~~
netcdf pres.sfc.2016 {
dimensions:
	lat = 73 ;
	lon = 144 ;
...
variables:
	...
	double time(time) ;
		...
		time:actual_range = 1893408., 1902168. ;
	float pres(time, lat, lon) ;
		...
		pres:actual_range = 49590.f, 109542.5f ;
...
$ file pres.sfc.2016.nc
~~~
{: .bash}
 
~~~
pres.sfc.2016.nc: NetCDF Data Format data
~~~
{: .output} 
